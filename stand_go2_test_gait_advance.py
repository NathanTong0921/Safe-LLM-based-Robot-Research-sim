...
def generate_trot_trajectory(t, phase_offset=0.0, step_height=0.05, step_length=0.025):
    ...
    while True:
        ...
        if runing_time < 3.0:
            ...
            for leg_name in ["LF", "RF", "LB", "RB"]:
                ...
                thigh, calf = leg_inverse_kinematics(x, z)
                foot_targets[leg_name] = (thigh, calf)

                # 打印输出
                print(f"[{leg_name}] x = {x:.4f}, z = {z:.4f}, thigh = {thigh:.4f}, calf = {calf:.4f}")
            
            hip_amp = 0.00  # 角度摆幅
            # hip_phase = sin(omega * (runing_time - 3.0) + phase_offset)
            ...
            for leg_name, (thigh, calf) in foot_targets.items():
                ...
                hip_offset = hip_amp * sin(omega * (runing_time - 3.0) + gait_phase_offsets[leg_name])
                cmd.motor_cmd[idx[0]].q = stand_up_joint_pos[idx[0]]+ hip_offset
